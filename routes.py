...
def chat():
    ...
    if products:
        ...
        product_text = "Hiện tại cửa hàng đang cập nhật sản phẩm."

    # Kịch bản (Prompt) bằng Tiếng Việt cho chatbot
    prompt = f"""
Bạn là một trợ lý mua sắm AI thân thiện và chuyên nghiệp của cửa hàng.
LUẬT LỆ BẮT BUỘC:
- CHỈ ĐƯỢC PHÉP TRẢ LỜI BẰNG TIẾNG VIỆT. Tuyệt đối không dùng tiếng Anh.
- Chỉ tư vấn dựa trên danh sách sản phẩm bên dưới, không tự bịa ra sản phẩm không có thực.
- Trả lời ngắn gọn, lịch sự. Không dùng markdown (không dùng ký tự * hoặc #).
